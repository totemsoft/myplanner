IF ( exists ( SELECT * FROM DBVersion WHERE CurrentVersion = 'FID.01.64' ) )
	RAISERROR( 'FID.01.64 already exists', 16, 1 ) 
	--WITH NOWAIT, SETERROR
GO



if exists (select * from dbo.sysobjects where id = object_id(N'dbo.vUserPersonClients') and OBJECTPROPERTY(id, N'IsView') = 1)
 drop view dbo.vUserPersonClients
GO
CREATE VIEW dbo.vUserPersonClients
 AS
...
if exists (select * from dbo.sysobjects where id = object_id(N'dbo.vUserManagmentDataProvider') and OBJECTPROPERTY(id, N'IsView') = 1)
 drop view dbo.vUserManagmentDataProvider
GO
CREATE VIEW dbo.vUserManagmentDataProvider
 AS

 SELECT  TOP 100 PERCENT a.UserPersonID AS UserPersonID, a.AdviserTypeCodeID AS AdviserTypeCodeID, a.LoginName AS LoginName, 
	...
	d.State AS State, d.DateCreated AS DateCreated, GETDATE() AS DateModified, e.ContactMediaID AS PhoneContactMediaID, 
 ...
 FROM    (SELECT	AddressID, AddressCodeID, StreetNumber, StreetNumber2, Suburb, Postcode, StateCodeID, CountryCodeID, DateCreated, DateModified, 
	...
	g RIGHT OUTER JOIN
	dbo.Person f INNER JOIN
	dbo.Person c INNER JOIN
	dbo.Person b INNER JOIN
	dbo.UserPerson a ON b.PersonID = a.UserPersonID ON c.PersonID = a.UserPersonID ON f.PersonID = a.UserPersonID INNER JOIN
	dbo.Person k ON a.UserPersonID = k.PersonID ON g.PersonID = f.PersonID ON l.PersonID = k.PersonID ON d.PersonID = b.PersonID LEFT OUTER JOIN
	  (SELECT     ContactMediaID, ContactMediaCodeID, Value1, Value2, ContactMediaDesc, DateCreated, DateModified, ObjectID1 PersonID
	    FROM          ContactMedia cm, Link ln
